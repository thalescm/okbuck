...
abstract class AndroidTarget extends JavaLibTarget {
    ...
    Scope getTest() {
        return Scope.from(
                project,
                unitTestVariant ? ImmutableSet.of(unitTestVariant.compileConfiguration) : ImmutableSet.of(),
    ...
    @Override
    Scope getApt() {
        return Scope.from(project, ImmutableSet.of(baseVariant.annotationProcessorConfiguration))
    }

    @Override
    Scope getTestApt() {
        return Scope.from(project, unitTestVariant
                ? ImmutableSet.of(unitTestVariant.annotationProcessorConfiguration)
                : ImmutableSet.of()
        )
    }
